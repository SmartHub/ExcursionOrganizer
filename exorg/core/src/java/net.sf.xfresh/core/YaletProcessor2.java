...
/* Adapted YaletProcessor class */

public class YaletProcessor2 {
    ...
    public void process(final HttpServletRequest req,
                        ...
                        final String source,
                        boolean transform) throws UnsupportedEncodingException, ServletException {
        req.setCharacterEncoding(encoding);
        res.setCharacterEncoding(encoding);

        log.info("Start process user request => {" + req.getRequestURI() + "}, remote ip => {" + req.getRemoteAddr() + "}, method=" + req.getMethod());
        final long startTime = System.currentTimeMillis();

        final InternalRequest internalRequest = yaletSupport.createRequest(req, "");

        final InternalResponse internalResponse = yaletSupport.createResponse(res);

        process(internalRequest, internalResponse, new RedirHandler(res), source, transform);
        log.info("Processing time for user request => {" + req.getRequestURI() + "} is " + (System.currentTimeMillis() - startTime) + " ms");
    }

    protected void process(final InternalRequest request,
                           final InternalResponse response,
                           final RedirHandler redirHandler,
                           final String source,
                           boolean transform) throws ServletException {
        try {
            //final InputSource inputSource = new InputSource(realPath);
            final InputSource inputSource = new InputSource(new ByteArrayInputStream(source.getBytes()));
            ...
            throw new ServletException("Can't process file ", e);
        } finally {
            try {
    ...
    private Transformer createTransformer(final String source) {
